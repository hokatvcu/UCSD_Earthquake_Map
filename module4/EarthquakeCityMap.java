	...
	private boolean isLand(PointFeature earthquake){
		for(Marker country:countryMarkers){
			if(isInCountry(earthquake, country)){
				isInCountry(earthquake, country);
				return true;
			}
		}
	...
	private void printQuakes(){
		for(Marker country: countryMarkers){
			...
			for(Marker quake: quakeMarkers){
				countryName = country.getStringProperty("name");
				String quakeCountry = (String) quake.getProperty("country");
				if(countryName.equals(quakeCountry)){
					landCounter += 1;
				}
			}
			if(landCounter > 0){
				System.out.print( countryName +  ": ");
				System.out.println(landCounter);	
			}
		}
		...
		System.out.println(oceanCounter);
	}
